...
library(tidyverse);library(magrittr);library(ggplot2);library(anytime);library(broom);library(glmnet);library(glmnetUtils)
```
...
One naive approach is to minimize $z_0(a, b) = \sum e_i = \sum (y_i - \hat{y_i})$, or the sum of all the **residuals** (errors) our model $y_i$ has with the true $y$ in the data. As it turns out, this is not an effective metric for a variety of reasons, and positive residuals may cancel out negative ones - in fact, for any linear regression that passes through the mean of our dataset, $(\bar{x}, \bar{y})$, $z_0(a,b) = 0$, regardless of $b$. Instead, we will use the **sum of squares** method: $z(a,b) = \sum(e_i)^2 = \sum (y_i - \hat{y_i})^2 = \sum (y_i - a - bx)^2$. This value is known as the **residual sum of squares** (RSS). 
...
Of course, although all of this math is important for understanding the linear regression model, in practice, R will do it all for you! Simply use the base R function, `lm`. `lm` uses a **formula** syntax, where the reponse variable is on the left and the explanatory variable(s) are on the right: `Y ~ X1 + X2 + X3 ...`. Alternatively, you can simply request all the explanatory variables with `Y ~ .`, where `Y` is the response variable column in your dataframe.

The complete function sytax is `lm(data, formula)`. Let's use linear regression to understand the relationship between petal lengths and sepal lengths in the `iris` dataset:
```{r}
...
To get even more information about the model you created, like its **residual standard error**, $\sigma = \sqrt{\frac{\textit{RSS}}{n-k-1}}$), or the $R^2$ (how much of the total variation in $Y$ is explained by our model $\hat{Y}$?), you can call `summary` on your model:
```{r}
...
```{r, echo = F}
nonlinear_dataset <- data.frame(x = rep(seq(-5, 10, .25), each = 5)) %>%
  mutate(y = rnorm(305, (2*x^3- 6*x^2 - 2*x - 14)/10, 10))
```
...
Of course, with our human eyes, we can clearly see that this is some sort of quadratic or cubic relationship. If only we could specify that we wanted to fit a polynomial, rather than linear model to our data...

**Polynomial regression** is an immediate extension of linear regression to account for polynomial relationships. If you recall from your theoretical math classes, polynomials are useful because they can approximate most functions arbitrarily well. So, with polynomial regression, our model becomes more complex, and therefore more suspectible to random **variations** in our data, but less rigid, therefore able to avoid **bias** a bit better. 
...
Our quadratic model performs better on every metric, with a higher $R^2$, and a smaller $\sigma$ (**sigma** is another name for the residual standard error).
...
Let's make a simple function, `generate_poly_glance`, that generates a `glance` dataframe for an $n^{\textit{th}}$-degree polynomial regression model applied to the `nonlinear_dataset`. Then, we use `map_dfr` to bind together all the rows into a single dataframe, and then graph the $\textit{RSS}$ ($\sigma^2(n-k-1)$) and $R^2$ for each model as a function of model complexity:
```{r}
...
models<- map_dfr(1:20, generate_poly_glance, .id = "Model Degree") %>%
  mutate(`Model Degree` = as.numeric(`Model Degree`),
         `Residual Sum of Squares` = sigma^2 * (nrow(nonlinear_dataset) - 2),
         `R-squared` = r.squared) 

models %>% select(`Model Degree`, `R-squared`, `Residual Sum of Squares`) %>% head(10)
```
```{r}
  ggplot(models, aes(`Model Degree`, `R-squared`)) +
  geom_point(size = 2) + geom_line(size = 2) 

  ggplot(models, aes(`Model Degree`, `Residual Sum of Squares`)) +
  geom_point(size = 2) + geom_line(size = 2) + scale_y_log10()
```

Hmm... the `R^2` value seems to just get better and better. In fact, it is a statistical theorem that $R^2$ will always increase with model complexity - one of the reasons $R^2$ is not an effective metric for model fit for complex or high-dimensional data. In fact, a metric called *Adjusted* $R^2$ was developed to adjust the $R^2$ to resist arbitrary inflation from adding more and more variables.
...
At the heart of machine learning is **model selection**. Essentially, you try to **tune** a hyperparameter (e.g. the degree of our polynomial regression) by training the data on part of the data, then validating its performance on a different part of the data. A **hyperparameter** is simply a paramater (like $\alpha$ or $\beta$), but it determines which *family* of model you're using - in this case, which degree polynomial our model is.
...
generate_poly_validation <- function(degree) {
  lm(training_set, formula = y ~ poly(x,degree)) %>% 
    ...
    summarise(`Residual Sum of Squares (Validation Set)` = sum((y - .fitted)^2)) %>%
...
```{r}
model_validations <- map_dfr(1:20, generate_poly_validation)
ggplot(model_validations, aes(`Model Degree`, `Residual Sum of Squares (Validation Set)`)) +
  geom_point(size = 2) + geom_line(size = 2)
head(model_validations, 10)
```

The one-degree and two-degree (linear and quadratic, respectively) models have too much error, diminishing the visibility of the other models. This is due to their **bias**, or strong rigidity, that prevents them from effectively capturing the underlying structure of the data.

Let's "zoom-in" on just the `degree > 2` models by filtering `model_validations` before graphing it:
```{r}
model_validations %>% filter(`Model Degree` > 2) %>% 
  ggplot(aes(`Model Degree`, `Residual Sum of Squares (Validation Set)`)) +
  geom_point(size = 2) + geom_line(size = 2) + scale_x_continuous(minor_breaks = 3:20)
```
...
In more advanced courses, you will learn a more effective method for validation called **cross-validation**. Essentially, cross-validation will conduct a similar validation process to that outlined above, but several times, and then average the results. That way, we get rid of even the sampling variance in our validation set.

For now though, we've gone through a lot! We've covered the objectives of data science, effective data visualization with `ggplot2`, and R's built-in function for linear and polynomial regression, `lm`, as well as how the `broom` package can be used to turn models into dataframes, and therefore graphs. We've also touched upon some of the essentials of machine learning, like the bias-variance tradeoff, how to do conduct validation, and the sum of square errors metric. 

# Mini-projects

There are two mini-projects for this tutorial, designed to give you practice with EDA and more advanced model selection. Find the full project specifications in the [mini-project section of the r3-workbook](r3-workbook.Rmd#mini-projects).
...
Especially in this age of "Big Data" buzz and the now ubiquitous nature of information, you will from time to time come across datasets with a very, very large number of explanatory variables (also called **features** or **predictors**). The challenge in making a model to capture the effects of these predictors on some response variable is we might grow unsure of which features are actually important to the underlying structure of the response variable. This is especially harmful because too many useless predictors can decrease your confidence in the structure of the data, especially if they fail to reveal any new information about the response. How can you select which predictors to include or exclude from your analysis when you have hundreds of potential variables? 

One problem with multivariate linear regression is that our error metric, the sum of squared errors (aka the **residual sum of squares**, $RSS$), is fairly simple. We could easily add a few more terms to it to penalize things like, the total "length" of our predicted slope parameters, $\sum |\hat{\beta_i}|^2$, or a "selection" penality, $\sum |\hat{\beta_i}|$. If we add these new penalties, along with **regularization hyperparameters** $\lambda_1$ and $\lambda_2$ to weight them, the matrix form of our objective function for elastic net regression is:

$$z_{\lambda_1,\lambda_2}(B) = \sum|y - XB|^2 + \lambda_1\sum|\hat{\beta_i}| + \lambda_2\sum|\hat{\beta_i}|^2$$

If we write $\lambda_1 = \alpha \lambda$ and $\lambda_2 = \frac{(1-\alpha)}{2} \lambda$, we arrive at our final parameterization of the objective function for elastic net regression:

$$z_{\alpha, \lambda}(B) = \sum|y - XB|^2 + \lambda\left(\alpha\sum|\hat{\beta_i}| + \sum|\hat{\beta_i}|^2\right)$$

Now, the trick is to pick various $\alpha_i$ and $\lambda_i$, train our $B$ on the training test, and then compute the $z_{(\alpha_i, \lambda_i)}$ for each $(\alpha_i,\lambda_i)$  for comparison. We will then choose the $\alpha^*$ and $\lambda^*$ that minimize our **validation error**, $z_{(\alpha, \lambda)}$. While this seems like a lot of work at first, the package we'll learn in R to do this actually finds the optimal $B$ and $\lambda$ for you. It's your job to find the best $\alpha$ between $0$ and $1$. 

To use elastic net regression in R, install the `glmnet` and `glmnetUtils` packages. The `cv.glmnet(formula, data, alpha)` function will find the model with the best $\lambda$ and $\beta$ for a given $\alpha$. Once you have such a model with `cv.glmnet`, use `predict(model, newdata, s = "lambda.min")` to find the predicted values for the response variable.

As a quick example, suppose we wanted to fit an ($\alpha = 0$) elastic net to predict the `price` of a diamond given its `carat`, `depth`, and `table`, and then wanted to graphically compare the predicted values with the actual diamond prices as well as the predicted values from a linear model. Note that in this example, we don't manually split our data into training and validation sets - you'll have to do that in your project. The following code is fairly advanced, but you should be able to understand all of it with a few read-overs. Feel free to email [`contact@arun.run`](mailto:contact@arun.run) with any questions about the code and its output.
```{r}
## Making elastic and linear models
diamond_model_elastic <- cv.glmnet(formula = price ~ carat + depth + table, data = diamonds, alpha = 1)
diamond_model_linear <- lm(formula = price ~ carat + depth + table, data = diamonds)

## Notice that the best lambda is 403.5063, called "lambda.min"
print(diamond_model_elastic)
glance(diamond_model_elastic)

## Comparing predictions with actual prices
diamond_predictions_elastic <- diamond_model_elastic %>% predict(newdata = diamonds, s = "lambda.min") %>% as.numeric
diamond_predictions_linear <- diamond_model_linear %>% predict(newdata = diamonds)
diamond_actual <- diamonds %>% pull(price)

## Create a dataframe of model outputs for each diamond
diamonds_diagnostic <- 
  data.frame(`Actual` = diamond_actual, `Predicted (lm)` = diamond_predictions_linear, `Predicted (glmnet)` = diamond_predictions_elastic, check.names = F) 

## Plot Model Outputs
diamonds_diagnostic %>% gather(Model, Prediction, - Actual) %>%
  ggplot(aes(Actual)) + 
  geom_line(aes(y = Prediction, col = Model), alpha = .4) +
  geom_line(aes(y = Actual), col = "black", linetype = "dashed") + facet_wrap(~ Model) +
  labs(x = "Diamond Price (Actual)", y = "Diamond Price (Predicted)", title = "Elastic Net vs Linear Model for Diamond Price Prediction") +
  theme(legend.position = "bottom")

## Plot Model Performances
diamonds_performance <- diamonds_diagnostic %>% 
  summarise(`RSS (lm)` = sum((`Predicted (lm)` - `Actual`)^2), `RSS (glmnet)` = sum((`Predicted (glmnet)` - `Actual`)^2)) %>%
  gather(Model, RSS) %>% mutate(Model = str_extract(Model, pattern = "\\(.*\\)") %>% str_sub(2, -2))
print(diamonds_performance)
```

Interestingly, both the elastic net model and the linear regression model seem to overvalue cheap diamonds and undervalue expensive diamonds, and the linear model actually performs slightly better. There are two reasons why this could have occured - linear regression might be better suited to this prediction task (*"better" algorithms aren't always better!*), or we could have chosen the wrong $\alpha$ (*note that we haven't used validation to select the optimal* $\alpha$!). 

This ends our brief introduction into elastic net regression. For more information on how to use `glmnet`, read the [official Quick Start guide](https://web.stanford.edu/~hastie/glmnet/glmnet_alpha.html). 
