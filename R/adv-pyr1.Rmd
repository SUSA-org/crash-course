...
We could have the loss be $0$ for correctly classifying the number, and $1$ for incorrect classification. However, this is a bit naive because we lose a lot of information on what the correct answer is. A better loss function for classification is **cross entropy loss**. The formula is a bit complex, but the formula is:
