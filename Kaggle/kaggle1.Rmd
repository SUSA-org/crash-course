...
# printer...
data.frame(x = seq(1, 10, .009) %>% extract(1:1000)) %>% mutate(y = x^2 + rnorm(1000, 0, 0.5)) %>%
  mutate(`x^2` = x^2) %>% gather(Variable, Value, - y) %>% ggplot(aes(Value, y, col = Variable)) + geom_point() + geom_smooth(method = "lm")
  facet_wrap(~ Variable, ncol = 1)  + labs(x = "", title = "Using x' = x^2 fixes our linear model.") + theme(legend.position = F)
